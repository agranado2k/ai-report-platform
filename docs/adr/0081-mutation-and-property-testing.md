...
- ~~A **labelled-PR** run.~~ **Landed** (issue #283): `.github/workflows/mutation-delta.yml` + `scripts/mutation-delta-ci.sh`. Adding the `mutation-check` label to a PR (or pushing to one that already carries it) runs the differential script and publishes the summary as exactly one PR comment, edited in place. It triggers on nothing else, holds `pull-requests: write` and no more, and is absent from `required_status_checks` — §1's "never on every push, and not as a required status check", made structural. Still open: the **periodic/scheduled** half, to track the score over time — that one needs a place to put the history, which nothing here provides.
