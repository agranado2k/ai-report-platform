...
- The iframe document carries its own **`Content-Security-Policy`** (a `<meta http-equiv>` tag, inserted as `<head>`'s first child, before the report's own `<style>`): `default-src 'none'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; base-uri 'none'`. This is the enforcing boundary against CSS-based exfiltration from the now-rendered `<style>` block — no fetch/XHR/WebSocket, no remote `@import`/`url(...)`-driven image or font loads, no rogue `<base href>` rewriting relative URLs. It is belt-and-braces on top of (not a replacement for) `sanitizeStyle`'s existing stripping of `url(...)`/`image-set(...)`/`expression(...)`/`@import` at the schema layer for any style value that round-trips through the editor.
