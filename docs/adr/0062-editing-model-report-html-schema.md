...
- **The fix is forward-only, and there is prior data loss.** Reports already saved through the editor have permanently lost their non-`<section>` ids, in both the HTML and the `_source.json` sidecar. Recovery is possible only from version history — pre-edit blobs are immutable in R2 — and may warrant a one-off for specific reports. Sidecars written before this amendment load unchanged (`id` defaults to `null`), pinned by a test.
