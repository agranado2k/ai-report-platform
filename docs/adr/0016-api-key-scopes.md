...
- **2026-08-01 — the scope-picker deferral (§Deferred) fired and the picker shipped.** `acl:write` became key-reachable (it gates `setAcl` / `grantWrite` / `revokeWrite` / `listWriteGrants`), meeting the "second scope gains key-reachable mutations" trigger. `settings.api-keys` now offers scope selection at mint time; `createApiKey` validates the selection with `makeScopes` against `KEY_ISSUABLE_SCOPES` (`reports:write`, `acl:write` — deliberately only the *enforced* subset) from the closed `Scope` vocabulary in `packages/domain/src/scope.ts`. The no-selection default stays `reports:write`. `reports:read` / `folders:write` remain documented-but-unenforced and are therefore not issuable; issue #224 tracks that enforcement gap.
