...
2. **Membership via Clerk** (invitations, membership records, the `<OrganizationSwitcher />` surface) mirrored through the webhook anti-corruption layer — consistent with ADR-0048's "Clerk is the identity source of truth". Note the existing webhook endpoint handles **only `user.deleted`** today (ADR-0054); the membership/org event handling is new plumbing this build adds, not existing capacity. No parallel membership table unless mirroring proves insufficient.
