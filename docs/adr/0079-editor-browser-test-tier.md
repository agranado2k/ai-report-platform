...
6. **The harness fixture must carry the CSS real reports carry — a fixture is part of the tier's fidelity boundary, not a convenience.** *(Added 2026-08-05, after the tier's first outing certified a broken build.)* The original `harness/report.html` shipped a four-line stylesheet chosen to make the geometry easy to reason about. Real reports ship a full generated stylesheet, and one of its rules — `html { scroll-behavior: smooth }`, present verbatim in `packages/report-html/src/fixtures/ai-readiness-report.html` — **changes the meaning of the code under test**: both `scrollIntoView({behavior: "auto"})` and ProseMirror's own `window.scrollBy()` mean "use the CSS `scroll-behavior`", so they are instant under the old fixture and animated-and-abortable under a real report. The tier was therefore green on code that was inert in production, which is the exact failure it was created to prevent (ADR-0062 Amendment 3, Decision 7's third-round correction). **The rule this establishes: when a fixture omits something every production document has, the tier is not testing production.** A fixture may be *smaller* than a real report; it may not be *different in kind*. Anything in the shell CSS that a browser consults while running the code under test — `scroll-behavior`, `overflow`, `scroll-margin`, `position: sticky`, `overflow-anchor` — belongs in the fixture, and the fixture carries a comment saying so.
...
- **The tier has now caught the thing it was built for, but only after its fixture was corrected.** With `html { scroll-behavior: smooth }` in the fixture, "the scroll survives ProseMirror's post-click caret reveal" fails on `c7ae920` (the shipped, production-broken code) by 602px and passes on the fix. That is the first time any automated check in this repo has been red for the reason production was broken. **The lesson is about the fixture, not the tier**: the tier's design was sound in PR #245 and it still certified a broken build, because a test tier is only as faithful as the document it runs on.
