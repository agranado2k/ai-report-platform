...
- **The primary client sends no key.** *(Closed 2026-08-07 — see the follow-up below.)* `apps/mcp/src/client.ts` sets only `accept`, `authorization` and `content-type` on every request — `uploadReport` documents that omission deliberately. So the mitigation "clients wanting exactly-once retry send an `Idempotency-Key`" does **not** currently apply to the agent-driven client ADR-0039 was written for. Concretely, via MCP: the 17 `unsound` operations have no retry protection, the six delete-shaped ones surface a `404` to the agent for a call that actually succeeded, and a retried `create-api-key` mints a second credential. Threading a key through the MCP client is the obvious follow-up and is **not** in this change.

### Follow-up (2026-08-07) — the two gaps the amendment left open

**The MCP client now sends a key.** `apps/mcp/src/client.ts` mints an `Idempotency-Key` for every non-GET request EXCEPT multipart uploads, which deliberately keep sending none so the server still derives one from the CONTENT — upload is a `sound` operation where that derived key IS the guarantee (identical bytes uploaded twice stay one version), and replacing it with a random per-attempt key would have quietly turned dedup into duplication and pairs it with a bounded transport retry (3 attempts; 408/429/5xx and network errors, with exponential backoff plus jitter and `Retry-After` honoured up to a 5s cap — never a CLIENT-FAULT 4xx, which a retry cannot fix). A `409 IdempotencyInFlight` is retried too, but only after a delay: the claim INSERT autocommits before the mutation's transaction, so an immediate retry meets our own first attempt still settling and is handed "request in flight" instead of the recorded response — the retry would bury the real error rather than recover from it. The key is minted ONCE per logical call and reused by each attempt, so it identifies the ATTEMPT rather than the payload: two genuinely separate calls with identical bodies get different keys. Deriving it from the body would have re-created, client-side, the exact defect this ADR's amendment removed server-side. `applyFolderSharingToReports` is excluded from retry entirely: ADR-0078 §10 has it deliberately ignore `Idempotency-Key` because it is a LOOP of per-report state sets, so a re-run leaves the end state correct but reports every report changed on the first attempt as `skipped` — the agent would be told `changed: []` for a run that changed N. Multipart uploads keep single-attempt behaviour — a `FormData` body is a one-shot stream in some runtimes, so replaying it is not guaranteed safe.

**Retention is swept on the existing scan-drain tick**, not behind a new route, secret and Cloudflare trigger. `IdempotencyStore.purgeExpired(olderThan, limit)` deletes a bounded batch, and `POST /internal/scan-drain` calls it after the drain, best-effort: a purge failure is logged and reported in the response, never escalated, because the drain gates reports becoming servable and retention does not. Standing up a second scheduled trigger for a job whose worst outcome is "the table is larger than it needs to be" would add operational surface with no reliability gain.

The purge is deliberately **not** load-bearing: `begin` already reclaims an expired row, so correctness never waits on the sweep having run. That separation is the direct lesson of finding 2 — this ADR once described a 24h window enforced by a sweep that did not exist, and records therefore replayed forever.
