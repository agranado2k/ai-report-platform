...
**Accepted residual (part 2):** the `smoke` job checks out the PR **head sha** (`github.event.pull_request.head.sha`) while the redeploy targets the branch **name** (`gitSource.ref = head.ref`, the branch's latest). Across *overlapping* pushes under `concurrency: cancel-in-progress: false`, an in-flight run's redeploy can build a newer tip than the sha its smoke checked out — deployed code and test definitions momentarily diverge. Accepted, not fixed: the divergence is narrow (serialized runs minimise overlap; the newer run corrects it), and the tempting fix (pin `gitSource.ref` to the head *sha*) would risk the deployment losing its **git-branch association**, so the git-branch-scoped isolated env (`gitBranch = $BRANCH_REF`) wouldn't apply → `/health` reports `isolated:false` → the fail-loud gate breaks the smoke. If it ever needs closing, capture the *deployed* commit sha from the redeploy response and check that out — do not change the redeploy's `ref`. (claude-review, PR #176.)
