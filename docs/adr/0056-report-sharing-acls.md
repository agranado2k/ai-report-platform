...
- **Owner access (any private mode):** the viewer is credential-free and can't recognise an owner, so the **app** (which holds the Clerk session) mints a slug-bound, ~24h **`owner`-claim** access token at `GET /reports/{slug}/open` — only after `getReport({orgId})` proves the signed-in org owns the report — and hands the owner to the viewer with `?access=`. The viewer trusts the signed `owner` claim and serves regardless of share mode (bypassing the mode-bound + allowlist membership/grant checks); the owner is not subject to their own report's gate. The dashboard's report link points at `/open`, so the owner opens their own private report in one click — no password / magic link.
