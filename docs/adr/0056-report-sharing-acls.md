...
- **`private` mode + private-by-default (supersedes the original default-public):** a fifth mode, **`private`** (owner-only), is now the **default** for every report with no `acls` row — sharing (`public`/`password`/`allowlist`) is an explicit opt-in via `set_acl`. The viewer serves a `private` report ONLY to an `owner`-claim token (see Owner access below); everyone else is denied (the unlock page shows a plain "this report is private" notice, no form). This reverses the Phase-1 default-public-on-missing: `rowToAcl(no row) ⇒ private`, `createReport ⇒ private`, and the `acls.mode` column default + `acl_mode` enum gain `private`. Rationale: a report platform should be private by default; the owner still reaches their own reports friction-free via owner-open. (Migration `0009`: `ALTER TYPE acl_mode ADD VALUE 'private'` + flip the column default.)
