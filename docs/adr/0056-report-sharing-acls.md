...
- **`private` mode + private-by-default (supersedes the original default-public):** a fifth mode, **`private`** (owner-only), is now the **default** for every report with no `acls` row — sharing (`public`/`password`/`allowlist`) is an explicit opt-in via `set_acl`. The viewer serves a `private` report ONLY to an `owner`-claim token (see Owner access below); everyone else is denied (the unlock page shows a plain "this report is private" notice, no form). This reverses the Phase-1 default-public-on-missing: `rowToAcl(no row) ⇒ private` and `createReport ⇒ private` — the default is **app-enforced** (the `acls.mode` column default stays `public` and is unused, since `set_acl` always writes an explicit mode). Migration `0009` only `ALTER TYPE acl_mode ADD VALUE 'private'` — we deliberately do **not** `SET DEFAULT 'private'`, because Postgres rejects using a newly-added enum value in the same `drizzle-kit migrate` transaction (it batches all pending migrations into one txn; #127). Rationale: a report platform should be private by default; the owner still reaches their own reports friction-free via owner-open.
- **Owner access (any private mode):** the viewer is credential-free and can't recognise an owner, so the **app** (which holds the Clerk session) mints a slug-bound, ~24h **`owner`-claim** access token at `GET /reports/{slug}/open` — only after `getReport({orgId})` proves the signed-in org owns the report — and hands the owner to the viewer with `?access=`. The viewer trusts the signed `owner` claim and serves regardless of share mode (bypassing the mode-bound + allowlist membership/grant checks); the owner is not subject to their own report's gate. The dashboard's report link points at `/open`, so the owner opens their own private report in one click — no password / magic link. **Owner = any member of the owning org** (gated by `getReport({orgId})`), not the individual creator — consistent with the org-scoped model (single-member personal orgs today). **Accepted trade-off:** unlike allowlist (revocation-C, denied on the next request), the `owner` token is **un-revocable for its TTL** — a member who leaves the org (or has access revoked) keeps serving until the token expires. The TTL (currently **24h**) bounds that window; because the dashboard re-mints on every click, it can be shortened (e.g. 1h) at ~no UX cost if a tighter org-departure revocation window is wanted.
