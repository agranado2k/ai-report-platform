...
- **Auth**:
  ...
  - **Interactive clients** (Claude Desktop/Code) use **Clerk OAuth** (built in PR 4): the server is an OAuth 2.1 resource server — it serves RFC 9728 protected-resource metadata at `/.well-known/oauth-protected-resource/mcp` (pointing at Clerk as the AS, derived from the publishable key) and replies `401 + WWW-Authenticate` to start discovery. It verifies the inbound Clerk OAuth access token via `@clerk/backend` `authenticateRequest({ acceptsToken: "oauth_token" })`, then **mints a short-lived Clerk *session* token** for that user and forwards THAT to `/api/v1` (OAuth-in → **session-token-out**). Spec-clean: the inbound OAuth token is never forwarded; the minted session token is a separate Clerk-issued credential `/api/v1`'s existing session path already accepts — so **no `/api/v1` change, no API-key minting, no schema change**. (Chosen over the spike's API-key-out: simpler, reuses the session path, no key-lifecycle/sprawl problem. `@clerk/mcp-tools` wasn't needed — verification + metadata are a thin `@clerk/backend` wrapper.) The MCP app gains its first secret, `CLERK_SECRET_KEY` (+ publishable key), split prod-live/preview-dev like the app; **fail-closed** — unset ⇒ OAuth off, `arp_` path unaffected.
  - **Operator setup (click-ops, both Clerk instances):** create a Clerk **OAuth application** on the dev + live instances, enable **Dynamic Client Registration**, set scopes + the `https://mcp.<apex>/mcp` resource. No Terraform resource exists for Clerk OAuth apps (dashboard-only) — an ADR-017 exception like the per-provider PATs. Then test via the MCP Inspector's OAuth mode.

## Spec-deviation note (token passthrough)

The MCP spec (rev 2025-11-25, §Security best practices) forbids an MCP server from **passing a client's token through** to an upstream API. Our headless path forwards the caller's **API key** to `/api/v1`. The spec never names API keys (its MUST-NOTs are written about OAuth access tokens), so this is outside the *letter* of the rule, but it matches the *spirit* of the passthrough anti-pattern. We accept it deliberately for a **single-vendor** setup (the MCP and `/api/v1` are both ours, the key's audience *is* our platform). The OAuth path (built, PR 4) is spec-clean: it mints a *separate* Clerk session token (OAuth-in → session-token-out) rather than forwarding the inbound OAuth token. Recorded so the API-key relay is a conscious, documented trade-off — not an oversight.
