...
- **`apps/mcp` — a remote, stateless Streamable-HTTP MCP server** (`@modelcontextprotocol/sdk` ≥1.26) on **Express**, deployed as a **Vercel Node serverless function** (`api/index.ts` exports the Express app; `vercel.json` rewrites all paths to it; no framework preset). Domain `mcp.<apex>`. One Terraform `vercel-app` module instance + a `mcp` CNAME, applied by CI/CD on merge.
...
- Implementation: `apps/mcp/src/{client,tools,server,app,env}.ts`, `apps/mcp/api/index.ts`. Infra: `infra/terraform/envs/prod/main.tf` (`vercel_mcp`) + `infra/terraform/envs/shared/main.tf` (`mcp` CNAME). Credential: ADR-0008. Contract: `docs/api/openapi.yaml`.
