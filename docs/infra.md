...
> **DNS is managed as-code** in the `cloudflare-zone` module via the `records` list assembled in `envs/shared/main.tf` (`app_view_records` + `clerk_records` + Resend's records). All app CNAMEs are **DNS-only (`proxied = false`)** — proxying breaks Vercel/Clerk CNAME flattening + cert issuance. **Clerk** custom-domain records live in `local.clerk_records` (`accounts` → `accounts.clerk.services`, `clerk` → `frontend-api.clerk.services`, `clk._domainkey`/`clk2._domainkey`/`clkmail` → the instance DKIM/mail targets). They apply with the shared env; afterwards re-run Clerk's domain **Verify**. Don't add Clerk records by hand in the Cloudflare dashboard — that drifts from this list. **When you re-domain the Clerk instance (e.g. an apex cutover), the email DKIM/mail cluster id changes** — re-copy the `clk._domainkey`/`clk2._domainkey`/`clkmail` targets from the Clerk dashboard into `local.clerk_records` in the same PR, or email verification stays red. The re-domain also changes the `pk_live` (it encodes the Frontend API host) — update `CLERK_PUBLISHABLE_KEY_PROD` + `CLERK_SECRET_KEY_PROD`, then **redeploy** the Vercel prod projects (the git deploy snapshots env *before* `apply-prod`, so the merge build ships stale env). And each **social-login provider** (Google, etc.) allow-lists the old `clerk.<olddomain>/v1/oauth_callback` in its own OAuth console — add the new `clerk.<newdomain>/v1/oauth_callback` (+ JS origins) there too, or sign-in fails with `redirect_uri_mismatch`.
