...
- **ReportSearchQuery** — the parameters of an org-wide report **search**: an optional case-insensitive `query` (matched as a literal substring against title + slug), an optional `folderId` filter, and `limit`/`offset` paging. An application read type. Spec: ADR-0036.
...
- **Search (reports)** — the dashboard's org-wide, newest-first, paged listing of `Report`s with an optional title/slug filter (`searchReports` use case → `ReportRepository.searchByOrg`, backed by the `(org_id, updated_at)` index). Org-scoped: a caller only ever sees their own org's reports.
