...
- **ApiKey** — a credential issued to a `User` that acts on their behalf when calling the HTTP API. `arp_live_`/`arp_test_` secret, hashed with **HMAC-SHA-256 + a server pepper** (ADR-0008 amends the spec's argon2id), prefixed for display, scoped per ADR-016. User-scoped (`acting_user_id`) so cross-org collaboration works. Landed for `/api/v1` (alongside Clerk sessions); the MCP server consumes it. Spec: ADR-0008.
