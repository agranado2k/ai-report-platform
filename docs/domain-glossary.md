...
- **ReportSummary** — a lightweight read projection of a `Report` for list views (the dashboard): `slug`, `title`, and `isPublished` (a clean version is live). NOT the full aggregate — listing must not load every `ReportVersion`'s manifest. An application read type (like a DTO), not an aggregate.
