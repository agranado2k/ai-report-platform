...
- **Resource (wire)** — the API/MCP shape of an entity (ADR-0053): a flat `snake_case` object with `object` (the type discriminator, e.g. `"report"`/`"folder"`), `livemode`, its `External Id`, and its fields. No envelope nesting (not JSON:API). Errors are the RFC-9457 `Problem` instead (ADR-0040).
...
- **livemode** — boolean on every wire `Resource` (ADR-0053): `true` on the live deployment, `false` on preview/dev (from `API_KEY_ENV`).
