...
| `CollaboratorGranted` | Reports & Folders (`GrantFolderAccessUseCase`) | Notifier (email + magic link) · AuditLogger | Grant created on a `Folder` (the `Folder` aggregate owns the grant chain). |
| `UserCreated` | Identity & Access (Clerk webhook ACL) | Reports & Folders (resolve pending `Collaborator` `grantee_email` → `UserId`) · AuditLogger | The cross-context hook that lets email-invited grants bind to a real `UserId` on first sign-in. |
