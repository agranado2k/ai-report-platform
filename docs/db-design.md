...
| `version_editability` | `editable`, `unsplittable`, `unparsable` (ADR-0080; migration `0021`). The UNKNOWN state is the column's NULL, deliberately **not** a value — a version nobody probed must not read as a verdict. Migration `0022` resets every stored `unsplittable` back to NULL: PR #255 changed the split predicate twice (`a645b27` body-less documents, `f34d606` scan-not-regex), so those verdicts came from a rule that no longer exists (see the `editability` row below) |
| `version_fidelity` | `lossless`, `lossy` (ADR-0090; migration `0023`). Whether the editor would KEEP a version's bytes, not whether it can open them — the orthogonal twin of `version_editability`, and deliberately NOT a fourth value of it, because `editable` + `lossy` is the state that matters and one enum cannot hold two bits. The UNKNOWN state is again the column's NULL |
...
| `editability` | `version_editability` NULL | **Editability** (ADR-0080): the editor's own open-time precondition (`splitShell`, then `parseBody` when there is no `_source.json` sidecar) run against THESE bytes at write time. NULL = UNKNOWN (never probed) — no default, so migration `0021` asserts nothing about the rows it cannot read. Immutable per version: `upsertVersions` refreshes only `scan_status` on conflict. **Recorded-at-write means it can go stale when the predicate changes**: ADR-0062 Amendment 4 made body-less documents splittable, so migration `0022` NULLs every `unsplittable` row rather than guessing a new verdict a migration cannot compute (it cannot read R2). Editing is unaffected either way — `loadEditableDocument` re-derives the split from the stored bytes on every open. The column is NOT badge-only: it is part of the wire contract (`packages/http/src/wire`, `resource.ts`, `write-response.ts`, `content-response.ts`) and is surfaced by the MCP tools, and it is a required (nullable) property of four `openapi.yaml` schemas — the content read carries it beside `fidelity` so report, version and content reads all answer the same pair. So the reset is visible to API/MCP consumers as `"editability": null` — contract-legal and intended, but a consumer branching on the literal string will see it change |
| `fidelity` | `version_fidelity` NULL | **Fidelity** (ADR-0090): whether a parse-then-serialise round trip through the `Report HTML schema` (`parseBody` + `serializeBody`, the editor's OWN functions) would keep THESE bytes, run at write time and compared through a normaliser (whitespace, attribute order, entity encoding, self-closing forms). Loss is measured by NAME PRESENCE, not occurrence count; that rule and its accepted cost are stated in ADR-0090 §1, which is the binding record for them. NULL = UNKNOWN (never probed) — no default, no backfill, for ADR-0080's reason: a migration cannot read R2. Probed ONLY when `editability` is `editable`; a version with a `_source.json` sidecar is `lossless` by construction (ADR-0062 §4) without parsing. Immutable per version, like `editability`. Part of the wire contract (`packages/http/src/wire`, `resource.ts`, `write-response.ts`, `content-response.ts`) and a required (nullable) property of the `Report`, `VersionSummary` and `ReportContent` `openapi.yaml` schemas. **Advisory, never authorization**: nothing gates on it and no upload is rejected for being lossy |

Indexes: `report_id`, `(report_id, version_no)` unique, `scan_status`.

`editability` and `fidelity` are metadata **about** the stored bytes, never a transformation
