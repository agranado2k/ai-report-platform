...
| **Last commit on main**| `737f16a` — Merge PR #401 (`chore/model-ids`): the capability-tier map became **two maps, one per agent harness** (Claude Code / Codex), the `reviewer` tier always dispatched to the other vendor (ADR-0084 amended); released **v4.8.0**. |
...
| **Active worktrees**   | **None** — `kit-update-0.20.0` (#398), `reviewer-self-implemented` (#399) and `model-ids` (#401) are all merged and pruned (`/worktree-cleanup` 2026-09-21/22; root `main` at `737f16a`), as were the earlier `docs-reconcile`, `claude-review-fix` and `scanner-rename`. Open PR **#400** (`fix/e2e-cap-preflight`, issue #372 — the Clerk membership-cap preflight behind the flaky preview smoke) is from a **separate session** and has one failing check; it owns its own worktree, not tracked here. |
...
- R2 bucket versioning — `TODO` in `modules/r2/main.tf` (revisit on a provider bump or wrap via the R2 API).
- **Follow-ups opened by the 2026-09-21 agent-harness wave (#401):** (a) an **eval tier for the worker prompts** under `.agents/prompts/` — the cross-vendor reviewer's one un-taken finding (shared invariant §3: deleting a prompt's two-axis split or its no-push boundary leaves every test green); (b) a **Codex-side CI reviewer**, to restore ADR-030's two-vendor intent in CI, where only the Anthropic action runs today; (c) a **cheaper Codex model for `mechanical`** — it currently takes the coder's, a recorded exemption the mapping test asserts on purpose; (d) the operator still needs `AGENT_SESSION_HARNESS=codex` in `~/.codex/config.toml` for Linux Codex sessions with network on (no marker otherwise).
- **Should re-upload to a soft-deleted slug resurrect the report?** `uploadReport`'s reUpload path has no `deletedAt` filter, so it currently does (surfaced by the #128 review). Decide: intended (document it) or a bug (add the guard). Related: issue #132 (create-folder accepts a soft-deleted parent).
...
- **The owner-view / artifact-parity epic (PRD #356) is the most recent big surface** — ADRs 0088–0092, all amendments; `GET /<slug>` stays byte-for-byte and `packages/headers` was untouched by 0089/0091/0092. The owner view lives at `view.<domain>/<slug>/view` (sandboxed iframe, no `allow-same-origin`); upload `warnings[]` and the per-version `Fidelity` verdict are the two write-time facts it added. See the 2026-09-09 → 2026-09-19 entries.
...
## 2026-08-05 — The anchor scroll shipped inert, because the browser pass was skipped

`c9a1134` (PR #243, ADR-0062 Amendment 3) landed link fidelity with a fully green
unit suite, and one half of it did not work at all in production. External links
opened a new tab; in-page anchor links did nothing. The operator measured it on
`view.centaurspec.com/<slug>/edit`: click a `#fragment` link, iframe `scrollY`
stayed at 32 — the caret-placement offset — with the target still 715px below the
fold, unchanged after 4 seconds. Calling `scrollIntoView({behavior:'auto',
block:'start'})` by hand on **the very same element in the very same page**
worked: `scrollY` 32 → 1127.5.
...
`anchorScrollTransaction` moves PM's caret onto the anchor (`posAtDOM` →
...
defect was not in any pure function, it was an ordering fact about a mounted
ProseMirror and a real browser, which the node tier structurally cannot observe.
Relying on remembering to dogfood is not a control, so `tests/browser/` now
...
`dpl_HkJhjvY9P…`) before any of the above was concluded, and the bug does NOT
reproduce in a harness without a competing scroll — which is precisely why it
survived review, CI, and a first round of local reproduction attempts.
...
### 2026-09-19 — Docs reconcile: ADR-0092 wording matches the scanner, and the diary re-orients (owner-view epic closed)

Two documentation-only fixes, no source touched.

**ADR-0092 §2 + the `Upload warning` glossary entry — wording reconciled to the code.**
...
only "reads". Per AGENTS.md ("the spec wins" / reconcile to the contract), the **wording** was
changed to match the code, not the reverse: both now read "reads or writes", noting the write
throws the same `SecurityError`. No behavior, no test, no source changed.

**Diary `Current state` block refreshed.** It was dated 2026-08-29 and predated the whole
owner-view / artifact-parity epic. Rewritten in place (table + Open questions + Memory
pointers; Update protocol kept) to record: **PRD #356 SHIPPED + CLOSED** (2026-09-19); the
five new ADRs **0088** (viewer CSP artifact-parity allowlist + `frame-ancestors 'self'`),
**0089** (owner view — framed byte-for-byte report under first-party chrome), **0090**
(Fidelity verdict at upload), **0091** (grantee read token), **0092** (owner-view fallback +
`sandbox-incompatible` upload warning); the current `main` HEAD `38a4455` (Merge PR #389 /
#385); the live surfaces otherwise unchanged and **no infrastructure touched by the epic**;
and the epic worktrees merged + pruned. Spec stays **rev 9** — ADR-0088–0092 are amendments
and needed no rev bump.

Worktree `docs-reconcile`, branch `docs/reconcile-adr0092-diary`.
...
**Spec contradiction, flagged per `AGENTS.md`'s rule (the spec wins; fix it in its next
revision):** `docs/spec.html`'s docs-trigger matrix row still reads `.claude/skills/**` or
`.claude/hooks/**` → `CLAUDE.md`; since ADR-0082 and this move the live row
(`constitution/local-workflow.md`) is `.agents/skills/**` (bridged into `.claude/skills/`)
or `.husky/**` → `AGENTS.md`. The bridge keeps the spec's paths resolving, so nothing is
broken — the wording is what lags. Also noted while reviewing: `docs/context-map.md` still
says the `Comment*` events are consumed by the `AuditLogger`; `docs/events.md` and
ADR-0070 are right (audit rows are synchronous, the events have no subscriber), and the
glossary's new Context map section follows them.

**Post-review fixes (PR #398, `/pr-iterate`):** the three-way merge of `/review-pr` had
dropped the shell-hazard audit's two rule bullets (the kit had not changed them between
releases, so the merge let this repo's older absence win) — restored from the kit; Agent 7
now stops reading a PR body at `<!-- explain-diff-appendix -->` (the kit's rule, which
`/implement` already relied on); the provenance file `LICENSE-mattpocock-skills.md` is the
kit's 0.20.0 copy plus this repo's own rows; `guards.config.sh`'s process surface scopes
`scripts/` to the top-level tooling + `docs-conformance/` rather than the whole tree (the
edited-tests surface already owns `scripts/test/`); the Biome override also silences the
linter on kit-verbatim files; the omitted end-to-end tests' orphaned imports are gone.
