...
| **Active worktrees**   | `worktree/commit-separation` (`chore/commit-separation`), `worktree/eval-smoke-tighten` (`chore/eval-smoke-tighten`), `worktree/tdd-guard-extract` (`chore/tdd-guard-extract`), `worktree/validator-hardening` (`chore/validator-hardening`) — all four kept by `/worktree-cleanup` for **uncommitted changes**, not because they are in flight. `worktree/review-workflow-adr0044` (`chore/review-workflow-adr0044`) — unmerged. `worktree/diary-merge-train` (`docs/diary-merge-train`) — this entry. The AI-SDLC Phase 4/5 worktrees, `contract-seam`, `clerk-e2e-hygiene`, `commit-write` and `commit-write-final` are all merged and pruned. |
