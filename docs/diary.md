...
| **Last commit on main**| `59b4264` — Merge PR #313 (`feat/report-content-read`, #312). 2026-08-29 batch: **#318** adopted **agentic-sdlc v0.12.0** (full conformance — the `skill-web`/`skill-paths` advisory gates + the `skills:` manifest; installed `/explain-diff` and wired its appendix into `/implement`; the docs-conformance `context.mjs`/`runner.mjs` fork survives, now 11 validators), **#317** fixed the positional flake in the `toolbar-formatting` Save e2e (select a plain word via a DOM Range, not a fixed pixel offset), **#312** shipped report content-read — `GET /api/v1/reports/{slug}/content` + the `reports_get_content` MCP tool (mirrors the `/diff` auth seam; `?version=`, `?include=source`; oversized-payload download-URL deferred to #319). All web-flow-signed, `migrate-db` + `release` green. Prior batch (2026-08-28) ended at `0c94e37` (#308/#302/#307/#311). |
...
| **Active worktrees**   | None — root checkout in sync with `origin/main`. `kit-update-0.12.0` (#318), `toolbar-e2e-selection` (#317) and `report-content-read` (#312/PR #313) merged and pruned by `/merge-train` + `/worktree-cleanup` on 2026-08-29. |
