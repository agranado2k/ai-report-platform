...
| **Last commit on main**| `e490b5b` — Merge PR #310 (`feat/preview-view-origin`, #307). Close-out batch after the Selection-toolbar epic + v0.10.0 migration: **#308** widened CI typecheck to the apps (PR #309, also fixed a latent app-tier type break on main), **#302** filled the capability-tier mapping + wired `/to-tickets`/`/implement` on top of the migrated resolver (ADR-0084), **#307** wired `VIEW_ORIGIN` on previews via stable Vercel git-branch aliases so the cross-origin editor-Save e2e is runnable again (ADR-0085; the app's strict CORS was NOT widened — the harness's Vercel bypass headers are scoped out of the `@browser` project instead). All web-flow-signed, `migrate-db` + `release` green. |
...
| **Active worktrees**   | None — root checkout in sync with `origin/main`. `capability-tiers` (#302) and `preview-view-origin` (#307) merged and pruned by `/worktree-cleanup` on 2026-08-28 (following `toolbar-formatting`/#303, `floating-composer`/#305, `agentic-sdlc-v0-10`/#304 on 2026-08-27). |
