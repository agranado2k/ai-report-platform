...
Feature: Selection toolbar formatting round-trip (real browser)
  ...
  # The formatting epic's single e2e round-trip (PRD #295 / ticket #297). The
  # browser tier (tests/browser/selection-toolbar.spec.ts, ADR-0079) already
  # proves the toolbar's behavior against the mounted editor hermetically —
  # what only THIS tier can prove is the full journey across the deployed
  # system: a real owner session (Clerk), the app→view edit-token hand-off,
  # the toolbar dispatching a real ProseMirror command in the deployed editor,
  # the cross-origin Save (view browser JS → app API, Bearer edit token,
  # ADR-0063 Phase 4), the new ReportVersion appearing on the API, and the
  # saved formatting surviving reassembly + the scan pipeline back onto the
  # live document.
  #
