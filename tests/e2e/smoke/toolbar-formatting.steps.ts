...
Given("I have opened that report in the unified editor", async ({ page }) => {
  ...
  // mints the edit token; its Location's ORIGIN is broken on previews
  // (VIEW_ORIGIN is prod-only), so lift `et=`/`oa=` out of it and navigate
  // the real browser straight to the deployed view preview instead.
