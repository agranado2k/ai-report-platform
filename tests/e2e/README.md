...
| Assertion mechanics | The shared-org check reads the listing through an **API key** the third identity mints via the write path (`POST /settings/api-keys`) — NOT a bare session GET: an org-less session's read path resolves the user's *oldest* Clerk membership, which for both fixtures is an unmirrored decoy/legacy org, so a session GET would fail regardless of the join outcome. The key's issuing org is the canonically-resolved one, making the listing deterministic. |
