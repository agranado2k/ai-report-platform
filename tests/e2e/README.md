...
| Decoy org (`arp-e2e-gold-decoy-org`) | The dev instance runs `force_organization_selection: true` (same as prod), and Clerk gives a **zero-membership** user a `pending` session (task `choose-organization`) whose JWT carries `sts: "pending"` — @clerk/backend treats that as signed-out, so every request 401s (the PR #222 CI failure). In prod the ADR-0074 webhook pre-joins the user; e2e previews get no webhook, so `ensureThirdFixtureUser()` creates what the forced task itself would: an **anchorless decoy org** with the user as creator. That makes sessions mint `active` AND faithfully reproduces the production duplicate-org shape — the app must ignore the decoy (no `publicMetadata.domain`, so the anchor scan never adopts it) and land the user in the canonical domain org. |
| Org (ADR-0068 §1) | Same `agranado.com` team domain as fixture 2 — that's the point: two identities, one domain, one org (despite the decoy). |
| Minting a session for it | `mintThirdTestSession()` — same `E2E_CLERK_SECRET_KEY` gate as the other fixtures; runs the find-or-create + decoy heal first. |
| Assertion mechanics | The shared-org check is two-fold. (1) Clerk-side: after gold's first upload, the step asserts gold holds a membership in the **anchored** `agranado.com` org (`findAnchoredOrgMembership` — BAPI read; its absence fails loud, proving the canonical chain didn't join). (2) App-side: gold's session is **re-minted with that org active** (`POST /v1/sessions` `active_organization_id`, verified against the live BAPI — the v2 token's `o.id` claim reaches the app's `getAuth` as the session org, exactly like a browser session after the forced task's org selection), then a bare `GET /api/v1/reports` must list BOTH fixtures' uploads. A session *without* an active org can't be used for the read: the read path resolves an org-less session via the user's *oldest* membership, which for both fixtures is an unmirrored decoy/legacy org. NOT `POST /settings/api-keys`: that dashboard action is cookie-session territory — a Bearer-token POST gets redirected to the sign-in HTML (the PR #222 round-2 failure). |
