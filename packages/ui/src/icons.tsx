...
/**
 * Inline SVG icon set. Geometry is Lucide's (lucide.dev, ISC) — vendored as
 * pure-SVG React components rather than pulling the `lucide-react` runtime
 * dependency, so the set stays CSP-safe and ADR-0050-clean (no icon-library
 * runtime, no Trusted-Types sink) while gaining Lucide's finished look and
 * 2px stroke. Each icon inherits `currentColor` and is sized via className
 * (e.g. `size-4`); decorative by default (`aria-hidden`) — give the
 * interactive parent the accessible label. The export NAMES are stable, so
 * replacing the earlier hand-drawn paths with Lucide's is zero call-site churn.
 */
type IconProps = ComponentProps<"svg">;

/** Shared wrapper — one place for the viewBox, stroke, caps, and a11y default. */
...
// ---- glyphs used across the app (names stable since the hand-drawn set) ----
...
/** Document / report row leading glyph. */
...
/** Edit — pencil; the dashboard's entry point into the report editor (ADR-0062). */
...
/** History — the report version history + visual diff entry point (ADR-0065). */
...
// ---- additions for the 2026 app shell + component sheet (T2, PRD #330) ----
