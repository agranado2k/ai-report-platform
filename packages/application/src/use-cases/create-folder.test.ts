...
async function setup() {
  ...
  return { folders, ids: new SequentialIdGenerator() };
}

describe("createFolder use case", () => {
  it("creates a folder under the org Root and persists it", async () => {
    const d = await setup();
    const r = await createFolder(d, { orgId: orgA }, { parentId: rootA, name: "Archive" });
  ...
  it("creates a nested folder under a non-Root parent", async () => {
    const d = await setup();
    const y = await createFolder(d, { orgId: orgA }, { parentId: rootA, name: "2024" });
    if (!y.ok) throw new Error("setup failed");
    const q = await createFolder(d, { orgId: orgA }, { parentId: y.value.id, name: "Q1" });
    expect(q.ok && q.value.parentId).toBe(y.value.id);
  });

  it("rejects nesting under another org's folder (NotAllowed)", async () => {
    const d = await setup();
    const r = await createFolder(d, { orgId: orgA }, { parentId: rootB, name: "Mine" });
  ...
  it("rejects a duplicate sibling slug (ValidationError)", async () => {
    ...
    const dup = await createFolder(d, { orgId: orgA }, { parentId: rootA, name: "docs" });
    expect(!dup.ok && dup.error.kind).toBe("ValidationError");
  });

  it("rejects an empty name (ValidationError)", async () => {
    const d = await setup();
    const r = await createFolder(d, { orgId: orgA }, { parentId: rootA, name: "   " });
  ...
  it(`enforces max nesting depth ${MAX_FOLDER_DEPTH} (Root = 0)`, async () => {
    ...
    for (let depth = 1; depth <= MAX_FOLDER_DEPTH; depth += 1) {
      const r = await createFolder(d, { orgId: orgA }, { parentId: parent, name: `level${depth}` });
      expect(r.ok, `depth ${depth} should be allowed`).toBe(true);
      if (!r.ok) return;
      parent = r.value.id;
    }
    // The next one (depth MAX+1) is rejected.
    const tooDeep = await createFolder(d, { orgId: orgA }, { parentId: parent, name: "tooDeep" });
