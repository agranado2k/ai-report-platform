...
import { type UploadActor, type UploadCommand, type UploadReportDeps, uploadReport } from './upload-report';
...
describe('uploadReport', () => {
  ...
  it('creates a report: 201-shape result, persisted, event + scan enqueued, blob stored', async () => {
    ...
    if (r.ok) {
      ...
      expect(r.value.result).toMatchObject({ slug: 'slug000001', version: 1, scanStatus: 'pending' });
    }
  ...
  it('passes a bundle pre-check failure straight through (e.g. SVG → 415)', async () => {
    ...
    const r = await uploadReport(deps, cmd({ upload: { filename: 'x.svg', bytes: new Uint8Array() } }));
    expect(r.ok).toBe(false);
  ...
  it('replays an identical retry (same key + content) without creating a duplicate', async () => {
    ...
    expect(first.ok && second.ok && first.value.result.slug === second.value.result.slug).toBe(true);
  ...
  it('rejects a cross-org re-upload (403)', async () => {
    ...
    const r = await uploadReport(deps, cmd({ actor: actor({ orgId: orgId('o2') }), updateSlug: 'slug000001' }));
    expect(r.ok).toBe(false);
