...
describe("deleteReport use case", () => {
  it("soft-deletes a report (excluded from listByOrg)", async () => {
    ...
    const r = await deleteReport({ reports }, { orgId: orgA }, { slug: slug("aaaaaaaaaa") });
  ...
  it("rejects a cross-org report with NotAllowed", async () => {
    ...
    const r = await deleteReport({ reports }, { orgId: orgB }, { slug: slug("bbbbbbbbbb") });
    expect(!r.ok && r.error.kind).toBe("NotAllowed");
  });

  it("rejects an unknown report with NotFound", async () => {
    const reports = new InMemoryReportRepository();
    const r = await deleteReport({ reports }, { orgId: orgA }, { slug: slug("cccccccccc") });
  ...
  it("rejects an already-deleted report with NotFound", async () => {
    ...
    await deleteReport({ reports }, { orgId: orgA }, { slug: slug("dddddddddd") });
    const again = await deleteReport({ reports }, { orgId: orgA }, { slug: slug("dddddddddd") });
