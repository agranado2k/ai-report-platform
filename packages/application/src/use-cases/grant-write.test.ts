...
describe("grantWrite use case (ADR-0060)", () => {
  it("requires the acl:write scope", async () => {
    ...
    const r = await grantWrite(
      { reports, grants, identities, audit, uow, ...idempotencyTestDeps() },
  ...
  it("re-granting the same email upserts in place (no duplicate)", async () => {
    const { reports, grants, identities, audit, uow } = await seed();
    await grantWrite({ reports, grants, identities, audit, uow, ...idempotencyTestDeps() }, ACTOR, {
      slug: SLUG as never,
      email: "grantee@x.com",
    });
    await grantWrite({ reports, grants, identities, audit, uow, ...idempotencyTestDeps() }, ACTOR, {
...
describe("grantWrite idempotency (ADR-0039)", () => {
  it("re-applies on an identical KEYLESS retry — no derived-key replay (#233)", async () => {
    ...
    const first = await grantWrite(deps, ACTOR, input);
