...
export function run(ctx) {
  ...
  for (const f of files) {
    ...
    if (!FILENAME_RE.test(f)) {
      v(
        "adr-filename",
        `Filename "${f}" must be NNNN-kebab-title.md`,
        "Four-digit zero-padded number, then a kebab-case title.",
      );
    }
    ...
    for (const section of ctx.config.adr.requiredSections) {
      if (!text.toLowerCase().includes(section.toLowerCase())) {
        v(
          ...
          `Add a "## ${section}" heading (MADR template).`,
        );
      }
    }
    ...
    if (!statusMatch) {
      ...
      const ok = ctx.config.adr.allowedStatuses.some((s) =>
        status.toLowerCase().startsWith(s.toLowerCase()),
      );
      if (!ok) {
        v(
          "adr-status-value",
          `Status "${status}" not in allowed vocabulary`,
          `Use one of: ${ctx.config.adr.allowedStatuses.join(", ")}.`,
        );
      }
    }
  }
