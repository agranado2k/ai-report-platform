...
export function run(ctx) {
  ...
  for (const f of ctx.list(ctx.paths.features, ".feature")) {
    ...
    if (phaseTags.length === 0) {
      v(
        "no-phase-tag",
        `No phase tag (one of ${ctx.config.featureTags.phases.join(", ")})`,
        'Tag the Feature with its phase, e.g. "@phase-1".',
      );
    } else if (phaseTags.length > 1) {
      v(
        "multiple-phase-tags",
        `Multiple phase tags: ${phaseTags.join(", ")}`,
        "A use-case belongs to exactly one phase.",
      );
    }
    for (const t of tags) {
