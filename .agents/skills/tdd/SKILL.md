...
> - **What counts as source, and what counts as its paired test**: `scripts/guards.config.sh`. Those two regexes are the machine-readable version of "this change needed a test". Keep them in step with `vitest.config.ts`'s include globs: a tree the runner covers but the guards' policy file doesn't is silently un-guarded.
...
*Adapted from `engineering/tdd` in [mattpocock/skills](https://github.com/mattpocock/skills) — MIT, see `.agents/skills/LICENSE-mattpocock-skills.md`. The project-context prelude above is this repo's; the four sidecars are verbatim; `deep-modules.md`'s two diagrams are redrawn as fenced diagram-language blocks, since craft rule §10 forbids character art.*
